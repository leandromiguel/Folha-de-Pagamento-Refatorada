...
import Exceptions.*;
import Informacoes.*;
import Informacoes.FormaPagamento.*;
import Main.InterfaceUsuario;

import java.util.ArrayList;

public class EmpregadoBase implements Empregado {
    private int codigo;
    ...
    public EmpregadoBase(int codigo, String nome) {
        this.codigo = codigo;
        diaPagamento = new ArrayList<>();
        setDiaPagamento(InterfaceUsuario.setDiaPagamentoMenu());
        setNome(nome);
        setFuncao();
        endereco = new Endereco(new EnderecoBuilder());
        setFormaPagamento();
        setSindicato();
    }
    ...
    public void setFormaPagamento() {
        int opcao = InterfaceUsuario.formaPagamentoMenu();
        switch (opcao) {
            case 1:
                this.formaPagamento = new ChequePorCorreios();
                break;
            case 2:
                this.formaPagamento = new ChequeEmMaos();
                break;
            case 3:
                this.formaPagamento = new Deposito();
                break;
        }
    }
    ...
    public String toString() {
        return "Código - " + this.codigo + "\n"
                ...
                + "Função - " + this.funcao + "\n"
                + endereco.toString()
                + "Método de Pagamento - " + this.formaPagamento.toString() + "\n"
