...
public class OperacoesEmpregado {
    ...
    private int findEmpregado(int codigo) {
        for (int i = 0; i < empregados.size(); i++)
            if (empregados.get(i).getCodigo() == codigo) return i;
    ...
    public void addEmpregado(int tipo) {
        if (empregados.size() == 10) System.out.println("Limite máximo de 10 funcionários atigido.");
            ...
            switch (tipo) {
                case 1:
                    empregados.add(new Horista(new EmpregadoBase(codigo, checkNome())));
                    break;
                case 2:
                    empregados.add(new Assalariado(new EmpregadoBase(codigo, checkNome())));
                    break;
                case 3:
                    empregados.add(new Comissionado(new Assalariado(new EmpregadoBase(codigo, checkNome()))));
                    break;
            }
            codigo++;
    ...
    public void setCartaoPonto(int codigo) {
        if (!InterfaceUsuario.undo()) {
            ...
            if (i != -1) {
                ...
                } else System.out.println("O funcionário selecionado não é do tipo horista.");
            }
        }
    }
    ...
    public void setResultadoVendas(int codigo) {
        ...
        if (i != -1) {
            ...
            } else System.out.println("O funcionário selecionado não é do tipo comissianado.");
        }
    }
    ...
    public void setTaxaServico(int codigo) {
        ...
        if (i != -1) {
            ...
            } else System.out.println("O funcionário selecionado não faz parte do sindicato.");
        }
    }
    ...
    public void alterarDados(int opcao, int codigo) {
        ...
        if (i != -1) {
            switch (opcao) {
                ...
                case 10:
                    if (empregados.get(i) instanceof Comissionado)
                        ((Comissionado) empregados.get(i)).setPorcentagemComissao();
                    else System.out.println("O funcionário escolhido não é do tipo comissionado.");
                    break;
                case 11:
                    if (empregados.get(i) instanceof Horista) ((Horista) empregados.get(i)).setHoristaSalario();
                    else System.out.println("O funcionário escolhido não é do tipo horista.");
                    break;
                case 12:
                    if (empregados.get(i) instanceof Assalariado) ((Assalariado) empregados.get(i)).setSalario();
                    else System.out.println("O funcionário escolhido não é do tipo assalariado.");
                    break;
            }
        }
    }
    ...
    private double checkDescontos(double total, int index) {
        for (int i = 0; i < empregados.get(index).getTaxaServico().size(); i++) {
            if ((total - empregados.get(index).getTaxaServico().get(i)) > 0) {
                ...
                System.out.println("Demais cobranças serão feitas na próxima folha uma vez que o salário do funcionário não pode ser negativo.");
                return total;
            }
        }
        return total;
        ...
        if (!InterfaceUsuario.undoPagamento(dia)) {
            for (int i = 0; i < empregados.size(); i++) {
                if (checkDiaPagamento(i, dia)) {
                    ...
                    if (empregados.get(i) instanceof Comissionado) {
                        total = ((Assalariado) ((Comissionado) empregados.get(i)).getEmpregado()).getSalario();
